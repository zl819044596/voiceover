...
function VoicePicker({
  ...
  const groups: VoiceGroup[] = useMemo(() => {
    ...
    if (fav.length)
      result.push({ key: "favorites", label: "Favorites", emoji: "❤️", voices: fav });
    if (clones.length)
      result.push({ key: "clones", label: "Clones", emoji: "🎙️", voices: clones });
      ...
      >
        <span
          className={cn(
            ...
            selectedYet ? "bg-white/20" : "bg-gray-50"
          )}
        >
          {avatarFor(v)}
        </span>
        <div className="min-w-0 flex-1">
          <p
            className={cn(
              ...
              selectedYet ? "text-white" : "text-gray-900"
            )}
          >
            {v.label}
            {v.cloned && (
              <span
                className={cn(
                  "ml-1 text-[8px]",
                  selectedYet ? "text-violet-200" : "text-violet-600"
                )}
              >
                🤖
              </span>
          ...
          <p
            className={cn(
              "truncate text-[9px]",
              selectedYet ? "text-violet-200" : "text-gray-500"
            )}
          >
            {v.cloned ? "My clone" : v.chinese || v.style}
          </p>
        </div>
        {/* Preview */}
        <button
    ...
    <div ref={rootRef} className="relative min-w-0 flex-1">
      ...
      <button
        ...
        className={cn(
          ...
          "hover:border-violet-300 focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-100"
        )}
      >
      ...
      {/* Dropdown panel */}
      {open && (
        <div className="absolute left-0 right-0 top-full z-30 mt-1 max-h-72 overflow-y-auto rounded-lg border border-gray-200 bg-white p-2 shadow-2xl shadow-gray-900/10">
          {/* Search */}
          <div className="relative mb-2">
            ...
            <input
              ...
              className="w-full rounded-md border border-gray-200 bg-white py-1 pl-6 pr-6 text-[11px] text-gray-900 placeholder:text-gray-400 focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-100"
            />
            {query && (
              <button
                onClick={() => setQuery("")}
                className="absolute right-1.5 top-1/2 -translate-y-1/2 rounded-full p-0.5 text-gray-500 hover:bg-gray-100 hover:text-gray-900"
              >
                <X className="h-3 w-3" />
              </button>
            )}
          ...
          {q ? (
            ...
            <div className="space-y-2">
                ...
                return (
                  <div key={g.key}>
                    ...
                    >
                      ...
                      <span className="ml-auto text-[9px] text-gray-500">{g.voices.length}</span>
                    </button>
                    {!isCollapsed && (
              ...
              {groups.length === 0 && (
                <p className="py-3 text-center text-[11px] text-gray-500">No voices available</p>
              )}
            </div>
          )}
        </div>
...
export function SpeakerPanel({
  ...
  return (
    <aside className="flex flex-col rounded-xl border border-gray-200 bg-white shadow-sm">
      {/* Header */}
      <div className="flex items-center gap-2 rounded-t-xl bg-gradient-to-r from-violet-600 to-fuchsia-500 px-4 py-3">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
            ...
            >
              <div className="mb-2 flex items-center gap-2">
                ...
                <input
                  ...
                  className={cn(
                    "w-full min-w-0 rounded-md border border-transparent bg-transparent px-1 py-0.5 text-sm font-semibold text-gray-900 outline-none transition-colors hover:border-gray-300 focus:border-violet-500 focus:bg-gray-50 focus:ring-2 focus:ring-violet-100"
                  )}
                />
                <button
                ...
                <button
                  ...
                  className="shrink-0 rounded-md p-1.5 text-gray-500 transition-colors hover:bg-red-50 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-30"
                >
              ...
              <input
                ...
                placeholder="Instruction (optional) e.g. cheerful and enthusiastic"
                className={cn(
                  "mt-2 w-full rounded-md border border-gray-200 bg-white px-2 py-1.5 text-[10px] text-gray-700 outline-none transition-colors placeholder:text-gray-400",
                  "focus:border-violet-500 focus:bg-gray-50 focus:ring-2 focus:ring-violet-100"
                )}
              />
            </div>
          );
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-1.5 rounded-lg border-2 border-dashed border-violet-200 px-3 py-2 text-xs font-medium text-violet-600 transition-colors hover:border-violet-300 hover:bg-violet-50 disabled:cursor-not-allowed disabled:opacity-40"
        >
      ...
      <div className="space-y-3 border-t border-gray-200 bg-gray-50 p-4">
        <div>
          <div className="mb-1 flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-violet-600">{speed.toFixed(2)}×</span>
          </div>
          <input
            type="range"
            min={0.5}
        ...
        <div>
          <div className="mb-1 flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-violet-600">{volume}</span>
          </div>
          <input
            type="range"
            min={0}
